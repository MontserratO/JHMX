...
img_conahcyt <- "img/logo_secihti.png"
img_colsan <- "img/logo_COLSAN-color.png"
text_titulo <- "Hidropesquisa"
text_right <- "Las capas base son de caracter público y \nse obtuvieron del INEGI.\nLa capa de puntos GMA fueron desarrolladas \npor el equipo de El Colegio de San Luis y la \ncolaboración de los usuarios de Hidropesquisa."
...
if ("LimiteEstatal" %in% capas_seleccionadas) {
  g3 <- g3 + geom_sf(
    data = lim_estatales, size = 1, fill = NA, aes(color = "Límites estatales")
  )
}
if ("EdosCinturonExtractivo" %in% capas_seleccionadas) {
  g3 <- g3 + geom_sf(
    data = edos_cinturon, size = 0.5, fill = NA, linetype = "dashed", aes(
...
if ("Cuencas" %in% capas_seleccionadas) {
  g3 <- g3 + geom_sf(
    data = cuencas, size = 1, fill = NA, aes(color = "Cuencas")
  )
}
if ("CuerposAgua" %in% capas_seleccionadas) {
  g3 <- g3 + geom_sf(
    data = cuerp_agua, size = 0.5, fill = "blue", aes(color = "Cuerpos de Agua")
  )
}
if ("Rios" %in% capas_seleccionadas) {
  g3 <- g3 + geom_sf(
    data = rios, size = 0.5, aes(color = "Rios")
  )
}
if ("Acueductos" %in% capas_seleccionadas) {
  g3 <- g3 + geom_sf(
    data = acueductos, size = 1, aes(color = "Acueductos")
  )
}
if ("TunelEmisorOriente" %in% capas_seleccionadas) {
  g3 <- g3 + geom_sf(
    data = tunel_emisor_or, size = 1, aes(color = "Tunel Emisor Oriente")
  )
}
if ("TunelEmisorCentral" %in% capas_seleccionadas) {
  g3 <- g3 + geom_sf(
    data = tunel_emisor_ce, size = 1, aes(color = "Tunel Emisor Central")
  )
}
if ("GranCanal" %in% capas_seleccionadas) {
  g3 <- g3 + geom_sf(
    data = gran_canal, size = 1, aes(color = "Gran Canal")
  )
}
...
gg_inset <- ggdraw() +
  ...
  draw_image(
    img_conahcyt, x = 0.74, y = 0.03, width = 0.21, height = 0.21
  ) +
  draw_image(
    img_colsan, x = 0.74, y = 0.08, width = 0.2, height = 0.2
  ) +
  draw_plot(
  ...
  draw_text(
    text_right, x = 0.74, y = 0.07, size = 6, hjust = 0, vjust = 0,
    color = "black"
  ) +
  draw_plot(
